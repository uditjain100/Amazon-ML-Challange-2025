...
def build_structured(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return out[["log_pack_of","log_value_num","log_value_per_pack","txt_len","txt_words","txt_digits","txt_upper","txt_digit_ratio","txt_upper_ratio","unit"]]
...
def make_regressor():
    try:
        ...
        return "HGBRegressor", HistGradientBoostingRegressor(
            max_depth=10, learning_rate=0.06, max_iter=1200,
...
def main():
    ...
    word_tfidf = TfidfVectorizer(lowercase=True, strip_accents="unicode", ngram_range=(1,3), min_df=3, max_features=100_000, sublinear_tf=True, stop_words="english")
    char_tfidf = TfidfVectorizer(analyzer="char_wb", ngram_range=(3,6), min_df=3, max_features=50_000, lowercase=True)
    text_pipe = Pipeline([
        ("tfidf", ColumnTransformer([("word", word_tfidf, "catalog_content"), ("char", char_tfidf, "catalog_content")], remainder="drop", sparse_threshold=1.0)),
        ("svd", TruncatedSVD(n_components=300, random_state=42))
    ])
    num_cols = ["log_pack_of","log_value_num","log_value_per_pack","txt_len","txt_words","txt_digits","txt_upper","txt_digit_ratio","txt_upper_ratio"]
    cat_cols = ["unit"]
    ...
    for run, (tr, va) in enumerate(rkf.split(train), start=1):
        ...
        pipe.fit(X_tr, y_tr)
        pred_va = np.clip(pipe.predict(X_va), 0.0, None)
        rows.append(dict(Model=model_name, Run=run, SMAPE=smape(y_va, pred_va), RMSE=math.sqrt(mean_squared_error(y_va, pred_va)), MAE=mean_absolute_error(y_va, pred_va), R2=r2_score(y_va, pred_va)))
    df_cv = pd.DataFrame(rows)
    df_cv.to_csv(CV_LOG_CSV, index=False)
    summary = df_cv.groupby("Model").agg({"SMAPE":"mean","RMSE":"mean","MAE":"mean","R2":"mean"}).reset_index().rename(columns={"SMAPE":"SMAPE_mean","RMSE":"RMSE_mean","MAE":"MAE_mean","R2":"R2_mean"})
    X_full = pd.concat([train[["catalog_content"]], Xs_tr], axis=1)
    pipe.fit(X_full, y)
    X_te = pd.concat([test[["catalog_content"]], Xs_te], axis=1)
    pred_test = np.clip(pipe.predict(X_te), 0.01, None)
